...
OUTPUT_DIR = ROOT / "outputs" / "checkpoints" / "mt5"
...
def require_libraries() -> None:
    try:
        ...
        raise SystemExit("Install transformers, datasets and sentencepiece before running mT5 training.") from exc


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fine-tune mT5 for English-to-Swahili translation.")
...
def main() -> None:
    ...
    def tokenize_batch(batch):
        # mT5 is text-to-text, so the task prefix tells it what to learn.
        source_text = [f"translate English to Swahili: {text}" for text in batch["english"]]
        inputs = tokenizer(source_text, max_length=args.max_source_length, truncation=True)
